...
\usepackage{latexsym,amsmath,amssymb,bm,euscript,eufrak}
...
%The main idea behind our constructions is the binding of hedgehogs to bosons, and proliferation of the resulting bound states. We introduce additional `spin' degrees of freedom into our model, and it is from these spins that hedgehogs will be defined. 
...
%We can imagine the case where the bosons are not bound to hedgehogs (this is a topologically trivial example). In this case we do know the quantum Hamiltonian, and we find that the symmetry operation which generates Eq.~\ref{ztwoeqn} is anti-unitary. 
...
In order to search for the exotic surface behavior predicted in Ref.~\onlinecite{SenthilVishwanath}, we first determine the surface phase diagram.
...
%If the surface is a superfluid, the current-current correlators will be independent of the volume of the surface, which in our system is $1/L$ times the volume of the bulk. 
...
{\it ii)} As $K$ is increased, hedgehogs acquire a large energy cost, and become gapped. This phase was studied in Refs.~\onlinecite{LesikSenthil} and \onlinecite{LesikAshvin}. It is known as the Coulomb phase because it has an emergent gapless photon and gapped excitations that carry charge $1/2$ and interact via a Coulomb interaction. 
...
%The above biasing term indirectly breaks the degeneracy in the $Q$ variables, leading to $Q=+1/2$ at one external monopole and $Q=-1/2$ at the other. With or without the biasing term, in our Monte Carlo the system gets stuck in one of the $J$ states. (and the biasing term selects avunique $J$ state) Therefore we have broken the problematic degeneracies and removed the obstacle to measuring the Witten effect. We would like to stress that the Witten effect is ordinarily defined for a single monopole, in the thermodynamic limit. The problems we have with degeneracies are artifacts of the fact that we are trying to measure a Witten effect in a finite-size system with two external monopoles. If the action were defined on an infinite system with only one monopole, these problems would not arise.
...
%We could for example use a very large value of $\gamma$, which would introduce extra charge at $\scripty{r}=0$. However, the extra charge would not wind around the system $\tau$ direction, and so it would be compensated by less charge nearby. Therefore measurements taken far from the monopole would be unaffected. 
...
%The topological phase has $\Psi_{\rm pair}$ condensed but no symmetries. 
%When all the coefficients in Eqs.~(\ref{sspin}) and (\ref{pairing}) are small, there is nothing which can order the spins or gap the bosons. Therefore we are in the photon phase, which conserves $U(1)_{\rm spin}$ and breaks $U(1)_{\rm boson}$. To get a pairing phase with no broken symmetries, we need to restore the $U(1)_{\rm boson}$ symmetry and condense $\Psi_{\rm pair}$, without breaking the $U(1)_{\rm spin}$ symmetry.  With this in mind, let us consider how the various terms in the action change the system.

%The $\beta_{\rm surf}$ term binds both $\nabla_\mu\phi_{\uparrow}$ and $\nabla_\mu\phi_{\downarrow}$ to $a_\mu$, which causes the magnetization in Eq.~(\ref{cp1mag}) to acquire an expectation value, which means that $z_\uparrow^\dagger z_\downarrow$ is condensed when $\beta_{\rm surf}$ is large. Therefore this term breaks the $U(1)_{\rm spin}$ symmetry, taking us to the molecular phase. We can see from Fig.~\ref{cp1surfphase} that the $K_{\rm surf}$ term does not change the symmetry of the system on its own. However, when it is combined with the $\beta_{\rm surf}$ term it can restore the $U(1)_{\rm boson}$ symmetry and condense $\Psi_{\rm pair}$, as required. The catch is that it also breaks $U(1)_{\rm spin}$ symmetry.

%Let us first consider the broader phase diagram in Fig.~\ref{cp1surfpair}.  In the Higgs Phase at large $\beta_{\rm surf}$ and large $K_{\rm surf}$, $z_\uparrow$ and $z_\downarrow$ are individually condensed, and therefore $\Psi_{\rm pair}$ is condensed with our without the $t_{\rm pair}$ term. In the Molecular Phase at large $\beta_{\rm surf}$ and small $K_{\rm surf}$, $z_\uparrow$ and $z_\downarrow$ are gapped, and the main effect coming from the $\beta_{\rm surf}$ and $K_{\rm surf}$ terms is strong attraction between oppositely charged spinons, with the result that $\Psi_{\rm mol} \sim z_\uparrow^\dagger z_\downarrow$ develops long-range order.  In this region the $t_{\rm pair}$ term, which acts to attract spinons with the same gauge charge, is overpowered by the other terms and $\Psi_{\rm pair}$ is gapped.  In the Photon phase at small $\beta_{\rm surf}$ and small $K_{\rm surf}$, all fields $z_\uparrow, z_\downarrow, \Psi_{\rm mol}$, and $\Psi_{\rm pair}$ are gapped; the gauge field fluctuations are sufficiently strong to scramble the $\Psi_{\rm pair}$ propagation.  

%Finally, increasing $K_{\rm surf}$ while keeping $\beta_{\rm surf}$ small suppresses the gauge field fluctuations and keeps the $z_\uparrow, z_\downarrow$ fields gapped, while $t_{\rm pair}$ is sufficiently large to condense $\Psi_{\rm pair}$ giving the new phase.  By the Higgs mechanism the new phase does not have a gapless photon.  Furthermore, since $\Psi_{\rm pair}$ is not charged under $U(1)_{\rm spin}$ or $U(1)_{\rm boson}$ , 
...
In the Appendix we derive the above properties by coupling a matter field to a system in which the monopoles of a compact electrodynamics model are bound to bosons. Such CQED$\times U(1)_{\rm boson}$ models allow changes of variables similar to those possible in two-dimensions\cite{FQHE}, which allow their properties to be readily determined. After the change of variables has been performed, we can add an additional matter field to the CQED model, and this gives us precisely the \cp~$\times U(1)_{\rm boson}$ model studied in this paper.
...
Here we consider a model whose degrees of freedom are compact quantum electrodynamics (CQED) residing on a (3+1)-dimensional cubic space-time lattice labeled by $r$ and bosons residing on the dual lattice labeled by $R$.  Topological excitations in the CQED are monopoles, which are quantum particles in three spatial dimensions, and our model realizes condensation of bound states of $d$ monopoles and $c$ bosons.  We will argue that when $d=1$, these phases are analogs of Bosonic Symmetry-Protected Topological phases for such CQED$\times$boson systems and have quantized cross-transverse response given by the integer $c$.  On the other hand, when $d > 1$ these phases are analogs of Bosonic Symmetry-Enriched Topological phases and have fractional cross-transverse response given by a rational number $c/d$; these phases also feature fractionalized Faraday line excitations of the CQED and fractionalized boson particle excitations, as well as non-trivial mutual statistics between the particle and line excitations.  
...
We then considered binding of the monopoles in the compact gauge field and physical bosons of the second $U(1)$ symmetry.  The crucial difference with the CQED$\times$boson model is the additional matter fields present in the \cp$\times$boson model.  We will compare the systems without and with such matter fields and will argue that the matter fields destroy the distinctions of the former model, except when protected by additional discrete symmetries.

%While the CQED$\times$boson setting may appear artificial, this problem is relevant to the problem of SPT and SET phases of bosons in (3+1)D.  Specifically, one approach to such phases in models with U(1)$\times$U(1) symmetry is to represent the first U(1) system as an easy-plane CP1 model which has two matter fields (``spinons'') coupled to a compact gauge field.  Here we can also consider binding of the monopoles in the compact gauge field and physical bosons of the second U(1) symmetry.  The crucial difference with the CQED$\times$boson model is the additional matter fields present in the CP1$\times$boson model.  We will compare the systems without and with such matter fields and will argue that the matter fields destroy the distinctions of the former model, except when protected by additional discrete symmetries.
...
\end{eqnarray}
\end{widetext}
The above action can be compared to the action of Eqs.~(\ref{sspin}) and (\ref{cdbind}) in the main text, compared to the main text the above action is missing the $\beta$ term which couples the gauge field to ``spinons''. We also allow the option of binding $c$ bosons to each monopole, and we bind the CQED sector to an external probing field $\hext_{\mu\nu}(r)$, and boson sector to a probing field $\Aext_\rho$. (note that this is different from the way that the probing fields are treated in the main text)
%For the CQED sector, we use $2\pi$-periodic compact gauge field variables $a_\mu(r)$ and use ``Villain cosine form'' for the $2\pi$-periodic Maxwell term, which is realized with the help of integer-valued placket variables $u_{\mu\nu}(r)$.  
%The CQED sector is coupled to an external probing field $\hext_{\mu\nu}(r)$.  
...
From variables $B_{\mu\nu}(r)$, we define monopole four-current $\cQ_\rho(R)$ as in Eq.~(\ref{mondef}) in the main text.
%\begin{equation}
%\cQ_\rho(R) \equiv \frac{1}{2} \epsilon_{\rho\sigma\mu\nu} \nabla_\sigma u_{\mu\nu} ~;
%\end{equation}
...
\end{equation}
%In the above equations and in what follows, we imply summation over repeated indices and use gradient symbols to denote appropriate lattice derivatives.  A precise definition on the lattice is 
%\begin{eqnarray}
%\cQ_\rho(R) &=& \frac{1}{2} \epsilon_{\rho\sigma\mu\nu} \left[ u_{\mu\nu}\left(R + \frac{\hat{\rho} + \hat{\sigma} - \hat{\mu} - \hat{\nu}}{2} \right) \right. \\
%&& \left. ~~~~~~~~~~~~~~ - u_{\mu\nu}\left(R + \frac{\hat{\rho} - \hat{\sigma} - \hat{\mu} - \hat{\nu}}{2} \right) \right] ~, ~~~
%\end{eqnarray}
%which relates a ``monopolicity'' three-form defined on cubes (e.g., specified by an oriented triple $[\hat{\sigma}, \hat{\mu}, \hat{\nu}]$) to a one-form defined on links (specified by a direction $\hat{\rho}$) of the dual lattice in (3+1)D.  
Note that thus defined $\cQ_\rho(R)$ are conserved currents satisfying $\sum_\rho \nabla_\rho \cQ_\rho(R) = 0$, and they also satisfy condition of zero total current in all directions: $\cQ_{{\rm tot}, \rho} \equiv \sum_R \cQ_\rho(R) = 0$ [using the current conservation, the last condition is equivalent to requiring zero current of $\cQ_\rho$ through any cut at fixed $R_\rho$, e.g., $\sum_R \delta_{R_\rho = 0} \cQ_\rho(R) = 0$].

For the boson sector, we use representation in terms of integer-valued conserved currents $\cJ_\rho(R)$, which satisfy $\sum_\rho \nabla_\rho \cJ_\rho(R) = 0$.  We also require that the total boson current is equal to zero for all directions, $\cJ_{{\rm tot}, \rho} = 0$.  The primed sum over $\cJ_\rho(R)$ in the partition sum signifies all such constraints.  The condition of zero total current is again convenient for precise treatment on finite systems [namely, for performing change of variables involving $\cJ$ and $\cQ$ currents, Eq.~(\ref{SL2Z}) below], while for bulk properties one can ignore these details.  The ``binding'' term parametrized by $\lambda$ is the key interaction in the action Eq.~(\ref{Sorig}) and wants to have bound states of $d$ monopoles and $c$ bosons when $\lambda$ is large [i.e., it is minimized when $(\cQ, \cJ) = (d, c) \times {\rm integer}$].  

The CQED$\times$boson system has conserved electric field lines in the CQED sector and conserved particles in the boson sector; these are probed by external fields $\hext_{\mu\nu}(r)$ and $\Aext_\rho(R)$ respectively.
The coupling of the boson current $\cJ_\rho$ to $\Aext_\rho$ is standard.  In the absence of the binding term, the coupling of the CQED sector to $\hext_{\mu\nu}$ is also standard; the additional piece in the binding term, while not important for much of the discussed long-distance physics, is the correct form keeping together the ``Dirac string'' $\uu_{\mu\nu}$ and the probing field $\hext_{\mu\nu}$. %(e.g., this gives standard form when the partition sum is expressed in terms of the integer-valued electromagnetic field tensor).

We first separate out the monopoles in the CQED sector as follows:
\begin{widetext}
\begin{eqnarray}
\sum_{\uu_{\mu\nu}(r) = -\infty}^\infty \!\!\!\!\!\! [\dots ] \!=\!\!\!
{\sum_{\cQ_\rho(R) = \frac{1}{2} \epsilon_{\rho\sigma\mu\nu} \nabla_\sigma \uu_{\mu\nu}^{(0)}(R)}}^\prime \sum_{V_\mu(r) = -\infty}^\infty \sum_{\MM_{\mu\nu} = -\infty}^\infty 
[\uu_{\mu\nu}(r) = \uu_{\mu\nu}^{(0)}(r) + (\nabla_\mu V_\nu - \nabla_\nu V_\mu)(r) + \delta_{r_\mu = 0} \delta_{r_\nu = 0} \MM_{\mu\nu} ]~.
\label{umn}
\end{eqnarray}
\end{widetext}
Here $\uu_{\mu\nu}^{(0)}(r)$ is an integer-valued field whose monopolicity gives $\cQ_\rho(R)$ and is treated as a fixed function of $\vcQ(R)$.  $V_\mu(r)$ and $\MM_{\mu\nu}$ are independent integer-valued fields, where the latter appear from careful treatment of the boundary conditions.  Schematically, the above arises by dividing all configurations of $\uu_{\mu\nu}(r)$ into classes defined by $\cQ_\rho(R)$ and establishing how to recover all members of a given class from one representative.  Furthermore, any $\cQ_\rho(R)$ satisfying $\sum_\rho \nabla_\rho \cQ_\rho(R) = 0$ and $\cQ_{{\rm tot}, \rho} = 0$ can be represented as above using integer-valued $\uu_{\mu\nu}(r)$, so the primed sum over $\cQ_\rho(R)$ can be viewed as signifying these constraints.  

A subtle point in the above is the redundancy in $V_\mu(r)$.  One way to address it precisely is as follows.  We can argue that from all links of the (3+1)D cubic lattice, we can select a subset of links such that we can take $V_\mu(r)$'s that are non-zero only on these links and that these $V_\mu(r)$'s are indeed independent integer-valued variables.  We can also argue that the original CQED theory can be equivalently formulated using compact gauge fields $a_\mu(r)$ that are non-zero only on exactly the same links.  We will assume this implicitly in all manipulations below.

We now operate with the constrained sums over the integer-valued currents $\cQ_\rho(R)$ and $\cJ_\rho(R)$, taken to be ``outside-most'' sums in the partition sum.  We change to new independent summation variables on each link:
\begin{equation}
\begin{array}{c}
\cP = a \cQ - b \cJ, \\
\cG = c \cQ - d \cJ,
...
\end{eqnarray}
We can again take $V^\prime_\mu(r)$, $\MM^\prime_{\mu\nu}$ as fixed functions of $\cP_\rho(R)$ and $\cG_\rho(R)$, and can also take $V^\prime_\mu(r)$ to be non-zero on the same links as $V_\mu(r)$ in Eq.~(\ref{umn}).  Now we can express $\uu_{\mu\nu}(r)$ in Eq.~(\ref{umn}) as
\begin{eqnarray}
\uu_{\mu\nu}(r) &=&  d \uu_{P, \mu\nu}^{(0)}(r) - b \uu_{G, \mu\nu}^{(0)}(r) \\
...
\tilde{a}_\mu(r) &\equiv& \frac{a_\mu(r)}{d} - \frac{2\pi k_\mu(r)}{d} ~, \\
...
&& ~~~ + i \sum_{R\rho} \left[c \left(\frac{1}{2} \epsilon_{\rho\sigma\mu\nu} \nabla_\sigma \tilde{u}_{\mu\nu} \right)(R) - a \cG_\rho(R) \right] \Aext_\rho(R) ~.
\end{eqnarray}
\end{widetext}
In this reformulation, we have a new CQED system represented by the compact gauge fields $\tilde{a}_\mu(r) \in [0, 2\pi)$ and a new boson system represented by the conserved current variables $\cG_\rho(R)$.  We will now argue that these new variables give a direct representation of gapped excitations in the topological phase obtained for small $K$ and $\lambda$.  

For a quick illustration, let us set $\Aext_\rho(R) = 0$ everywhere.  In this case, we can perform summation over $\tilde{u}_{\mu\nu}(r)$ independently on each placket and obtain the Villain cosine for the combination
\begin{eqnarray}
...
\end{eqnarray*}
where in the first line we used $ad - bc = 1$, and the manipulation is such as to separate out the same combination of $\tilde{\uu}_{\mu\nu}$ and $\uu_{G, \mu\nu}^{(0)}$ as in the $K$-term in Eq.~(\ref{SKlambda}).
...
\end{eqnarray}
\end{widetext}
We see that the formally introduced $\tilde{F}_{\mu\nu}(r)$ can be interpreted as integer-valued electromagnetic field tensor variables conjugate to the compact $\tilde{a}_\mu(r)$ variables.  Integrating out $\tilde{a}_\mu(r)$ gives ``Maxwell equations in free space'', $\sum_\nu \nabla_\nu \tilde{F}_{\mu\nu}(r) = 0$, while integrating out $\tilde{\gamma}_{\mu\nu}$ gives the condition of zero total field for each component, $\sum_r \delta_{r_\mu = 0} \delta_{r_\nu = 0} \tilde{F}_{\mu\nu}(r) = 0$.  For small $K$, the quantum lines whose worldsheets are represented by $\tilde{F}_{\mu\nu}(r)$ are gapped, while for small $\lambda$ the quantum particles whose worldlines are represented by $\cG_\rho(R)$ are gapped, so we have managed to express the partition sum completely in terms of gapped excitations in this phase.  From the coupling of $\tilde{F}_{\mu\nu}$ to $\hext_{\mu\nu}$ we explicitly see that the new lines carry fraction $1/d$ of the unit electric field strength, while from the coupling of $\cG_\rho$ to $\Aext_\rho$ we see that the new particles carry charge $1/d$ of the original bosons.  The coupling between $\tilde{F}_{\mu\nu}$ and $\uu_{G, \mu\nu}^{(0)}$ encodes $2\pi b/d$ statistical interaction between the new elementary line and particle excitations.  Finally, the last term $i \frac{c}{4\pi d} \sum \epsilon_{\rho\sigma\mu\nu} \Aext_\rho \nabla_\sigma \hext_{\mu\nu}$ is a property of the ``vacuum'' in these variables and represents a kind of cross-transverse response in this phase, which we see is quantized to the rational number $c/d$ in appropriate units.


We can now make the connection to the in the \cp$\times$boson model in the main text, which we will refer to as $U(1)_{\rm spin} \times U(1)_{\rm boson}$ model.  We introducde additional ``spinon'' matter fields coupled to the compact gauge field, schematically
\begin{eqnarray}
\delta S &=& S_{\rm spinons}[J_\up, J_\dn] + i \sum_{r\mu} [J_{\up, \mu}(r) + J_{\dn, \mu}(r)) a_\mu(r) \\
&=& S_{\rm spinons}[J_\up, J_\dn] + i d \sum_{r\mu} [J_{\up, \mu}(r) + J_{\dn, \mu}(r)) \tilde{a}_\mu(r) ~,
\end{eqnarray}
where $J_{\up, \mu}(r)$ and $J_{\dn, \mu}(r)$ are integer-valued spinon currents, and in the last line we dropped all contributions of the form $i 2\pi \times$integer.  All manipulations leading to Eq.~(\ref{S_FG_wtildea}) do not touch the gauge field $\tilde{a}_\mu(r)$ and remain valid also in the presence of the spinon matter.  Now, since the original electric field lines have sources and sinks, the probing field $\hext_{\mu\nu}$ ceases to be meaningful and will be dropped; in particular, the characterization using rational $c/d$ for the cross-transverse response collapses.  On the other hand, the coupling of the new $\cG$ particles to $\Aext$ remains unaffected and we see that they carry $1/d$ fractional charge with respect to $U(1)_{\rm boson}$.  Furthermore, upon integrating out the $\tilde{a}_\mu(r)$ fields, we now have
\begin{equation}
